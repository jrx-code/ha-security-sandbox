...
def export_sarif(report: dict) -> dict:
    """Export report as SARIF 2.1.0 (Static Analysis Results Interchange Format).

    SARIF is a standard JSON format for static analysis tools, supported by
    GitHub Code Scanning, GitLab SAST, Azure DevOps, and most CI/CD platforms.
    """
    severity_to_sarif = {
        "critical": "error",
        "high": "error",
    ...
    for f in report.get("findings", []):
        ...
        if rule_id not in rules:
            rules[rule_id] = {
                ...
                "defaultConfiguration": {
                    "level": severity_to_sarif.get(sev, "note"),
                },
                "properties": {
                    "security-severity": str(severity_to_level.get(sev, 1.0)),
                },
            }
        ...
        if f.get("code"):
            result["properties"] = {"code_snippet": f["code"]}
        ...
        "runs": [
            {
                "tool": {
                    "driver": {
                        ...
                        "informationUri": report.get("repo_url", ""),
                        "version": "0.17.0",
                ...
                "properties": {
                    ...
                    "component_name": report.get("name", ""),
                },
            },
        ],
    }
