...
def create_job(job_id: str, name: str, repo_url: str) -> None:
    conn = _get_conn()
    conn.execute(
        "INSERT OR REPLACE INTO jobs (id, name, repo_url, status, created_at) VALUES (?, ?, ?, 'running', ?)",
        (job_id, name, repo_url, datetime.now().isoformat()),
    )
    conn.commit()
